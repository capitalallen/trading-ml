...
def preprocessing(outfolder):
    ...
    #get label file 
    labels = gtd.get_labels_no_side(outfolder)
...
p1 = Process(target=preprocessing,args=(outfolder[0],))
p1.start() 
p1 = Process(target=preprocessing,args=(outfolder[1],))
p2.start()
p1 = Process(target=preprocessing,args=(outfolder[2],))
p3.start()
p1 = Process(target=preprocessing,args=(outfolder[3],))
p4.start()
p1 = Process(target=preprocessing,args=(outfolder[4],))
p4.start()
